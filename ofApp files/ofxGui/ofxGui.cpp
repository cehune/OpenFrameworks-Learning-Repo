...
void ofApp::setup(){
	...
	gui.add(label.setup("label", "celines label lol"));
	...
	gui.add(vec2Slider.setup("vec2 slider", 
		ofVec2f(0, 0), ofVec2f(0, 0), ofVec2f(ofGetWidth(), ofGetHeight())));
	gui.add(vec3Slider.setup("vec3 slider",
		ofVec3f(100, 150, 90), ofVec3f(0, 0, 0), ofVec3f(255, 255, 255)));
	gui.add(vec4Slider.setup("vec4 slider", ofVec4f(0, 0, 0, 0), ofVec4f(0, 0, 0, 0),
...
void ofApp::draw(){
	if (button) {
		ofSetColor(ofRandom(vec3Slider->x),
			ofRandom(vec3Slider->y), ofRandom(vec3Slider->z));
	}
